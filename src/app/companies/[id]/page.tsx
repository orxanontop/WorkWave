...
export default function CompanyDetailPage() {
  ...
  if (isLoading) return <div className="container-app py-8"><div className="skeleton h-8 w-64 mb-4"/><div className="skeleton h-32 w-full"/></div>;
  if (!company) return <div className="container-app py-8 text-center"><p className="text-gray-500">Company not found</p></div>;
  ...
  return (
    <div className="container-app py-8">
      ...
      {/* Jobs */}
      <h2 className="text-xl font-bold text-gray-900 mb-4">Open Positions ({company._count?.jobs || 0})</h2>
      {company.jobs?.length === 0 ? (
        <div className="card p-8 text-center text-gray-500">No open positions at the moment</div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
                    ...
                    <span className="badge badge-gray text-xs">{JOB_TYPES.find(t => t.value === job.jobType)?.label}</span>
      ...
      {company.reviews?.length > 0 && (
        <>
          <h2 className="text-xl font-bold text-gray-900 mb-4 mt-8">Reviews ({company.reviewCount})</h2>
          <div className="space-y-4">
            {company.reviews.map((review: any) => (
              <div key={review.id} className="card p-5">
                ...
                {review.pros && <p className="text-sm text-green-600 mb-1"><strong>Pros:</strong> {review.pros}</p>}
                {review.cons && <p className="text-sm text-red-500"><strong>Cons:</strong> {review.cons}</p>}
