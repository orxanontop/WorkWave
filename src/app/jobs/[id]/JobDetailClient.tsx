    ...
    <div className="container-app py-8">
      <div className="max-w-4xl mx-auto">
        <div className={mounted ? 'animate-slide-up' : 'opacity-0'}>
          ...
          <div className="card p-6 sm:p-8 mb-6 animate-slide-up stagger-1 opacity-0">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-4 text-sm text-gray-500">
                  ...
                  <span className="badge badge-gray">
                    {WORK_MODELS.find((wm) => wm.value === job.workModel)?.label}
                  </span>
                  <span className="badge badge-gray">
                    {EXPERIENCE_LEVELS.find((el) => el.value === job.experienceLevel)?.label}
                  </span>
                </div>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 mt-6 pt-6 border-t border-gray-200">
              ...
              <button className="btn btn-ghost">
                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3.367 3.0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                </svg>
                {t('jobDetail.share')}
              </button>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              <div className="card p-6">
                ...
                <dl className="space-y-3">
                  <div>
                    ...
                    <dd className="text-sm font-semibold text-green-600 mt-0.5">{formatSalary(job.salaryMin, job.salaryMax)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 uppercase tracking-wide">{t('jobDetail.jobType')}</dt>
                    <dd className="text-sm text-gray-900 mt-0.5">{JOB_TYPES.find((jt) => jt.value === job.jobType)?.label}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 uppercase tracking-wide">{t('jobDetail.workModel')}</dt>
                    <dd className="text-sm text-gray-900 mt-0.5">{WORK_MODELS.find((wm) => wm.value === job.workModel)?.label}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 uppercase tracking-wide">{t('jobDetail.experience')}</dt>
                    <dd className="text-sm text-gray-900 mt-0.5">{EXPERIENCE_LEVELS.find((el) => el.value === job.experienceLevel)?.label}</dd>
                  </div>
                  {job.industry && (
                    <div>
              ...
              <div className="card p-6">
                ...
                <Link href={`/companies/${job.company.id}`} className="btn btn-secondary w-full text-sm">{t('jobDetail.viewCompany')}</Link>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="card p-6 sm:p-8 max-w-lg w-full animate-slide-up">
            ...
            <div className="mb-6">
              ...
              <textarea value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)} className="input min-h-[120px]" placeholder={String(t('jobDetail.applyModal.coverLetterPlaceholder'))} />
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={() => setShowApplyModal(false)} className="btn btn-secondary">{t('jobDetail.applyModal.cancel')}</button>
            </div>
          </div>
        </div>
      )}
